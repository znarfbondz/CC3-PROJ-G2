"""
dashboard.py  —  Student Information System
All data is persisted in students.db via db.py.
"""
...
class Dashboard:
    ...
    def __init__(self, root, username=None, on_logout=None, users=None):
        ...
        # users dict kept for credential-change compatibility
        self.users = users if users is not None else db.get_all_users()

        self.root.title(f"Dashboard - {username}" if username else "Dashboard")
        ...
        self._edit_sid: str | None = None   # student_id being edited
    ...
    def _style_tree(self):
        ...
        s.configure("Treeview", font=("Segoe UI", 10), rowheight=34,
                    background=self.T["CARD"], fieldbackground=self.T["CARD"],
                    foreground=self.T["TEXT"], borderwidth=0)
        s.configure("Treeview.Heading", font=("Segoe UI Semibold", 10),
                    background=self.T["ACCENT"], foreground="#ffffff",
    ...
    def _show_dashboard(self):
        ...
        tf = self._card(inner); tf.pack(fill="x", padx=20, pady=(0,14))
        self._style_tree()
        ...
        tk.Label(inner, text="Recent Activity", font=("Arial", 12, "bold"),
                 bg=self.T["BG"], fg=self.T["TEXT"]).pack(
            anchor="w", padx=28, pady=(16,4))
        lf = self._card(inner); lf.pack(fill="x", padx=20, pady=(0,20))
    ...
    def _show_form(self, prefill: dict | None = None):
        self._clear_content()
        # _edit_sid holds the original student_id when editing
        self._edit_sid = prefill.pop("_edit_sid", None) if prefill else None
        is_edit = self._edit_sid is not None
        self._header(self._content,
                     "✏️   Edit Student" if is_edit else "➕   Add Student")
        _, inner = self._scrollable(self._content)

        card = self._card(inner); card.pack(padx=40, pady=25, fill="x")
        ...
        body = tk.Frame(card, bg=self.T["CARD"], padx=28, pady=16); body.pack(fill="x")
        ...
        bf = tk.Frame(card, bg=self.T["CARD"], padx=28, pady=18); bf.pack(fill="x")
    ...
    def _save_record(self):
        try:
            ...
            if not all(c.isalnum() or c in "-_" for c in sid):
                raise ValueError("Student ID may only contain letters, digits, hyphens, or underscores.")
            ...
            if contact:
                ...
                if not clean_c.isdigit():
                    raise ValueError("Contact number may only contain digits, '+', '-', and spaces.")
            ...
            for subj, key in self.GRADE_SUBJS:
                ...
                try:
                    ...
                    raise ValueError(f"Grade for {subj} must be a number between 0 and 100.")

            # Duplicate ID check (DB-level)
            if db.student_id_taken(sid, exclude_id=self._edit_sid):
                raise ValueError(f"Student ID '{sid}' is already in use.")

            # Soft duplicate name+course warning
            full = f"{ln} {fn}".lower()
            for s in self.students:
                existing = s["name"].lower().replace(", ", " ")
                if (existing == full and s["course"] == crs
                        and s["student_id"] != self._edit_sid):
    ...
    def _show_records(self, term=""):
        ...
        tk.Label(tb, text="🔍 Search:", font=("Arial", 10),
        ...
        for row, s in enumerate(shown):
            # use student_id as iid so we can retrieve it on selection
            self._tree.insert("", "end", iid=s["student_id"],
    ...
    def _selected_sid(self) -> str | None:
        ...
        if not sel:
            ...
            return None
        return sel[0]   # iid == student_id

    def _student_by_sid(self, sid: str) -> dict | None:
        return next((s for s in self.students if s["student_id"] == sid), None)

    def _view_details(self):
        sid = self._selected_sid()
        if sid is None: return
        s = self._student_by_sid(sid)
        if s is None: return
        ...
        status_colors = {"Regular": self.T["SUCCESS"],
                         ...
                         "LOA": self.T["DANGER"]}
        for lbl, val in [
            ("Name",       s["name"]),      ("Student ID", s["student_id"]),
            ("Course",     s["course"]),     ("Year Level", s["year_level"]),
        ...
        for subj, key in self.GRADE_SUBJS:
            ...
            val = s.get(key, "0.00")
            try: gcol = self.T["SUCCESS"] if float(val) >= 75 else self.T["DANGER"]
            except ValueError: gcol = self.T["TEXT"]
            tk.Label(gr, text=val, font=("Arial", 10, "bold"),
                     bg=self.T["ROW_ODD"], fg=gcol).pack(side="left")
        self._btn(body, "  Close  ", win.destroy, bg=self.T["ACCENT"]).pack(pady=12)

    def _update_student(self):
        sid = self._selected_sid()
        if sid is None: return
        pre = dict(self._student_by_sid(sid))
        pre["_edit_sid"] = sid
    ...
    def _refresh_ctree(self):
        ...
        for i, c in enumerate(self.courses_data):
            ...
            self._ctree.insert("", "end", iid=c["code"],
    ...
    def _course_dialog(self, title: str,
        ...
        def save():
            try:
                ...
                if db.course_code_taken(code, exclude_code=old_code):
        ...
        bf = tk.Frame(body, bg=self.T["CARD"]); bf.pack(fill="x", pady=(10,0))
    ...
    def _edit_course(self):
        ...
        if not sel:
            ...
            return
        code = sel[0]   # iid == course code
        c    = next((x for x in self.courses_data if x["code"] == code), None)
        if c:
            self._course_dialog("✏️ Edit Course", prefill=c, old_code=code)
    ...
    def _delete_course(self):
        try:
            ...
            msg = (f"Delete course '{code} — {c['name']}'?"
                   + (f"\n\n⚠ {enrolled} student(s) still enrolled in this course."
                      if enrolled else ""))
            if not messagebox.askyesno("⚠ Confirm Delete", msg):
                return
            db.delete_course(code)
            self._log(f"Deleted course '{code} — {c['name']}'")
            self.courses_data = db.load_courses()
    ...
    def _apply_theme_setting(self):
        ...
        self._apply_theme(); self._build_main(); self._show_settings()
    ...
    def _change_credentials(self):
        try:
            ...
            if not cur:      raise ValueError("Current password is required.")
            if not new_user: raise ValueError("New username cannot be blank.")
            if len(new_user) < 3: raise ValueError("Username must be at least 3 characters.")

            # Verify against DB
            if not db.verify_login(self.current_user, cur):
                raise PermissionError("Current password is incorrect.")
            if new_pw or conf:
                if len(new_pw) < 6: raise ValueError("New password must be at least 6 characters.")
                if new_pw != conf:  raise ValueError("New passwords do not match.")
            if new_user != self.current_user and db.user_exists(new_user):
            ...
            if new_user != self.current_user:
                self._log(f"Username changed from '{self.current_user}' to '{new_user}'")
            if new_pw:
                self._log("Password updated")
            self.current_user = new_user
            messagebox.showinfo("✅ Success", "Credentials updated successfully!")
            self._build_main(); self._show_settings()
        except ValueError as e:
